...
SELECT   MAX (DECODE (job, 'CLERK', ename, NULL)) "职员名称",
         MAX (DECODE (job, 'CLERK', empno, NULL)) "职员编号",
         MAX (DECODE (job, 'ANALYST', ename, NULL)) "分析人员名称",
         MAX (DECODE (job, 'ANALYST', empno, NULL)) "分析人员编号",         
         MAX (DECODE (job, 'MANAGER', ename, NULL)) "经理名称",
         MAX (DECODE (job, 'MANAGER', empno, NULL)) "经理编号",                  
         MAX (DECODE (job, '老板', ename, NULL)) "老板名称",
         MAX (DECODE (job, '老板', empno, NULL)) "老板编号",         
         MAX (DECODE (job, 'SALESMAN', ename, NULL)) "销售人员名称",
         MAX (DECODE (job, 'SALESMAN', empno, NULL)) "销售人员编号"         
